...
Translate a hypothesis into the smallest reversible domain change while preserving client
compatibility, deterministic backtests, frozen defaults, and a direct path to isolated paper.
...
Read `AGENTS.md`, `SESSION.md`, the target strategy/config,
...
2. Define the hypothesis, unchanged behavior, changed behavior, rollback config, and acceptance
   tests before editing. A brief task-local contract is enough for a paper candidate.
...
8. Iterate locally: implement, run paired realistic backtests, fix defects, then run the defined
   lookahead and anti-overfitting checks. Hand off only for an independent audit or promotion/default
   decision; do not wait for generic specialist receipts to prepare isolated paper.
...
- Run focal tests, full pytest, compileall, build, Ruff ratchet, and inspect diffs.
...
- A passing candidate may be labelled `PAPER_CANDIDATE_READY`; that is not default/live approval.
