...
LEDGER = ROOT / ".v7-independent-replication-timestamp" / "certified_trades.csv"


def _config(tmp_path: Path):
    spec = tmp_path / "docs" / "v7_frozen_candidate.json"
    spec.parent.mkdir(parents=True)
    spec.write_text((ROOT / "docs" / "v7_frozen_candidate.json").read_text(encoding="utf-8"), encoding="utf-8")
...
def test_paper_adapter_is_dependency_isolated_and_setup_is_inactive(tmp_path: Path):
    ...
    assert "core.exchange" not in source and "okx" not in source.lower() and "place_order" not in source
    assert config.active is False and config.allow_shorts is False and config.promotion_allowed is False
...
def test_exact_six_operation_replay_parity_and_duplicate_suppression(tmp_path: Path):
    ...
    assert Decimal(result["final_cash"]) == Decimal("54002022.18728089349690")
    with LEDGER.open(newline="", encoding="utf-8") as handle:
        first = next(csv.DictReader(handle))
    assert CertifiedPaperAdapter(config).replay_operation(first)["status"] == "duplicate_intent"


def test_restart_after_each_persisted_operation_resumes_without_duplicate_fill(tmp_path: Path):
    config = _config(tmp_path)
    with LEDGER.open(newline="", encoding="utf-8") as handle:
        rows = list(csv.DictReader(handle))
    for row in rows:
        assert CertifiedPaperAdapter(config).replay_operation(row)["status"] == "reconciled"
...
def test_snapshot_requires_completed_utc_four_hour_inputs(tmp_path: Path):
    ...
    bar = SnapshotBar(at, Decimal("1"), Decimal("1"), Decimal("1"), Decimal("1"), Decimal("1"))
    snapshot = StrategySnapshot(at, (bar,), {}, Decimal("1"), Decimal("0"))
    assert adapter.decide(snapshot, TargetIntent("intent", target_base_pct=Decimal("1"))) is not None
...
def test_synthetic_operational_drills_fail_closed_without_real_state(tmp_path: Path, reason: str):
    adapter = CertifiedPaperAdapter(_config(tmp_path))
...
def test_candle_drills_reject_stale_conflicting_and_out_of_order_batches(tmp_path: Path):
    at = datetime(2026, 1, 1, 8, tzinfo=timezone.utc)
    for candles, reason in (([], "missing_required_candle"),
                            ([{"timestamp": "2026-01-01T00:00:00+00:00"}], "stale_data"),
                            ([{"timestamp": "2026-01-01T07:00:00+00:00"}, {"timestamp": "2026-01-01T06:00:00+00:00"}], "out_of_order_candle"),
                            ([{"timestamp": "2026-01-01T07:00:00+00:00", "close": "1"}, {"timestamp": "2026-01-01T07:00:00+00:00", "close": "2"}], "conflicting_duplicate_candle")):
...
def test_daily_report_is_read_only_and_never_calls_pending_an_executed_fill(tmp_path: Path):
    (tmp_path / "docs").mkdir()
    (tmp_path / "docs" / "v7_frozen_candidate.json").write_text(
        (ROOT / "docs" / "v7_frozen_candidate.json").read_text(encoding="utf-8"), encoding="utf-8"
    )
